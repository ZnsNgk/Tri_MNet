...
class PyramidPoolingTransformer(nn.Module):
    def __init__(self, img_size=64, patch_size=4, in_chans=1, num_classes=5, embed_dims=[64, 128, 320],
                 num_heads=[1, 2, 5], mlp_ratios=[8, 8, 4], qkv_bias=True, qk_scale=None, drop_rate=0.,
        ...
        self.heads = nn.Linear(embed_dims[2], num_classes)
...
def p2t_tiny(pretrained=False, **kwargs):
    model = PyramidPoolingTransformer(
        patch_size=4, embed_dims=[64, 192, 384], num_heads=[2, 4, 8], mlp_ratios=[8, 8, 4], qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), depths=[2, 9, 3],
        **kwargs)

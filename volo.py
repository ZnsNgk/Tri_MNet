...
class VOLO(nn.Module):
    ...
    def __init__(
        ...
        self.heads = nn.Linear(embed_dims[-1], self.num_classes)
...
@register_model
def volo_d1(pretrained=False, pretrained_cfg=None, **kwargs):
    """ VOLO-D1 model, Params: 27M """
    model = VOLO(layers=(4, 4, 8, 2), embed_dims=(192, 384, 384, 384), num_heads=(6, 12, 12, 12), **kwargs)
    return model

if __name__ == "__main__":
    model = volo_d1(False, img_size=64, in_chans=3)
    print(model)
